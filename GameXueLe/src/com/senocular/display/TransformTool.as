package com.senocular.display
{
	 ...
	 */
	public class TransformTool extends Group
	{
		...
		private var toolSprites:Group = new Group();
		private var lines:Group = new Group();
		private var moveControls:Group = new Group();
		private var registrationControls:Group = new Group();
		private var rotateControls:Group = new Group();
		private var scaleControls:Group = new Group();
		private var skewControls:Group = new Group();
		private var cursors:Group = new Group();
		private var customControls:Group = new Group();
		private var customCursors:Group = new Group();
		...
		private var _controlSize:Number = 15;
		...
		public function set target(d:UIComponent):void
		{
...
//			_moveCursor.addReference(_moveControl);
			//隐藏注册点
			_registrationControl.mouseEnabled = false;
			_registrationControl.mouseChildren = false;
			_registrationControl.visible = false;
			...
			// adding cursors
//			addToolControl(cursors, _moveCursor, false);//去掉移动光标
		...
		private function addToolControl(container:Group, control:TransformToolControl, interactive:Boolean = true):void
		{
...
//				customControls.removeChild(control);
				customControls.removeElement(control);
				return control;
			}
			return null;
		}
...
//				customCursors.removeChild(cursor);
				customCursors.removeElement(cursor);
				return cursor;
			}
			return null;
		}
		 ...
		 * @see removeControl
		 */
		public function setSkin(controlName:String, skin:UIComponent):void
		{
		 ...
		 * @see removeControl
		 */
		public function getSkin(controlName:String):UIComponent
		{
			var control:TransformToolInternalControl = getControlByName(controlName);
			return control.skin;
		}
		...
		private function getControlByName(controlName:String):TransformToolInternalControl
		{
			...
			var containers:Array = [skewControls, registrationControls, cursors, rotateControls, scaleControls];
...
//			updateRegistration()
		...
		private function endInteractionHandler(event:MouseEvent):void
		{
			...
			GameDispatcher.dispatchEvent(GameEventConst.TRANSFORM_VERTEX_DATA);
			if (!_livePreview)
			{
				// update target if applicable
				apply();
...
//					addChild(toolSprites);
					addElement(toolSprites);
				}
			} else if (isChild)
			{
//				removeChild(toolSprites);
				removeElement(toolSprites);
			}
		}
		...
		private function updateControlContainer(container:Group, enabled:Boolean):void
		{
...
//					toolSprites.setChildIndex(container, 0);
					toolSprites.setElementIndex(container, 0);
				} else
				{
//					toolSprites.addChildAt(container, 0);
					toolSprites.addElementAt(container, 0);
				}
			} else if (isChild)
			{
				// removed if disabled
//				toolSprites.removeChild(container);
				toolSprites.removeElement(container);
			}
		}
...
//			_target.parent.setChildIndex(_target, index);
			Group(_target.parent).setElementIndex(_target, index);
...
//				parent.setChildIndex(this, index);
				Group(parent).setElementIndex(this, index);
			}
		}
...
class TransformToolInternalCursor extends TransformToolCursor
{
	...
	public var icon:Group = new Group();
